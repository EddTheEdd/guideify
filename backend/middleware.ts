...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    if (token) {
      ...
      const hasRequiredPermissions = await checkPermissionsForPath(
        path,
        userPermissions,
        pagePermissions
      );
...
async function checkPermissionsForPath(
  path: string,
  userPermissions: any,
  pagePermissions: any
) {
  ...
  if (pagePermissions[path]) {
    return pagePermissions[path].every((permission: any) =>
  ...
  for (const route in pagePermissions) {
    if (route.includes(":")) {
      ...
      if (regex.test(path)) {
        return pagePermissions[route].every((permission: any) =>
          userPermissions.includes(permission)
        );
      }
    }
